...
describe("custom VSCode contexts using @sap/artifact-management tags", () => {
  ...
  function MODULE(opts: Omit<Module, "type" | "name">): Module {
    return {
      name: basename(opts.path),
      ...opts,
      type: "N/A",
  ...
  function ITEM(opts: Omit<Item, "type" | "ref" | "name">): Item {
    return {
      name: basename(opts.path),
      ...opts,
      type: "N/A",
  ...
  function PROJECT_API_WRAPPER(projects: Project[]): ProjectApiRead[] {
