...
import { AnalyticsWrapper, BASClientFactory, BASTelemetryClient } from "../src/usage-report/usage-analytics-wrapper";
...
describe('AnalyticsWrapper', () => {
    ...
    beforeEach(() => {
        ...
        getBASTelemetryClientStub = sandbox.stub(BASClientFactory, 'getBASTelemetryClient');
    ...
    describe('createTracker', () => {
        it('should create a tracker', () => {
            ...
            expect(getLoggerStub().info.calledOnceWith(`SAP Web Analytics tracker was created for SAPOSS.app-studio-toolkit`)).to.be.true;
    ...
    describe('traceProjectTypesStatus', () => {
        ...
        it('should report project types status correctly', () => {
            ...
            expect(mockClient.report.calledWith('Project Types Status', { projectType: 'CAP', devSpacePackName }, { projectTypeQuantity: 2 })).to.be.true;
            expect(mockClient.report.calledWith('Project Types Status', { projectType: 'UI5', devSpacePackName }, { projectTypeQuantity: 1 })).to.be.true;
            expect(mockClient.report.calledWith('Project Types Status', { projectType: 'MDK', devSpacePackName }, { projectTypeQuantity: 3 })).to.be.true;
            expect(mockClient.report.calledWith('Project Types Status', { projectType: 'FE', devSpacePackName }, { projectTypeQuantity: 1 })).to.be.true;
            expect(mockClient.report.calledWith('Project Types Status', { projectType: 'HANA', devSpacePackName }, { projectTypeQuantity: 1 })).to.be.true;
            expect(mockClient.report.calledWith('Project Types Status', { projectType: 'LCAP', devSpacePackName }, { projectTypeQuantity: 2 })).to.be.true;
            expect(mockClient.report.calledWith('Project Types Status', { projectType: 'BASEmpty', devSpacePackName }, { projectTypeQuantity: 1 })).to.be.true;
        
            expect(getLoggerStub().trace.callCount).to.equal(7);
            expect(getLoggerStub().trace.calledWith("SAP Web Analytics tracker was called", {eventName: 'Project Types Status'})).to.be.true;
        });
