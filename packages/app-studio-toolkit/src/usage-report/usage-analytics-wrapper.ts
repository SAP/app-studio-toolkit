  ...
  export class AnalyticsWrapper {
    ...
    private static report(opt: {
      ...
      if (process.env.LANDSCAPE_ENVIRONMENT) {
        void AnalyticsWrapper.getTracker().report(opt.eventName, { ...opt.properties }, { ...opt.measurements });
    ...
    public static traceProjectTypesStatus(
      ...
      try {
        ...
        for (const type in projects) {
          ...
          AnalyticsWrapper.report({ eventName, properties: {projectType, devSpacePackName }, measurements: {projectTypeQuantity: projects[type]} });
        }
      } catch (error: any) {
        getLogger().error(error);
      }
