	...
	function (
		...
		Opa5.createPageObjects({
			onTheAppPage: {
				actions: {
							...
							actions: [
								function (oList) {
									...
									var oListItem = oList
										...
										.getItems()[0];
									this._triggerCheckboxSelection(
										oListItem,
										bSelected
									);
								}.bind(this),
					...
					_triggerCheckboxSelection: function (oListItem, bSelected) {
						...
						if (
							...
							oPress.controlAdapters["sap.m.CustomListItem"] =
								"selectMulti-CB";
				...
				assertions: {
					iShouldSeeTheItemBeingAdded: function (
						iItemCount,
						sLastAddedText
					) {
						return this.waitFor({
							...
							errorMessage:
								"List does not have expected entry '" +
								sLastAddedText +
								"'.",
						});
					},
					iShouldSeeTheLastItemBeingCompleted: function (bSelected) {
						return this.waitFor({
							...
							success: function () {
								Opa5.assert.ok(
									true,
									"The last item is marked as completed"
								);
							},
							errorMessage: "The last item is not disabled.",
						});
					},
					iShouldSeeAllButOneItemBeingRemoved: function (
						sLastItemText
					) {
						return this.waitFor({
							id: sItemListId,
