...
describe("cf-local unit tests", () => {
  ...
  describe("bindLocalServices", () => {
    ...
    it("ok:: tags are not provided", async () => {
      cliMock
        .expects("execute")
        .withExactArgs(
          ["bind-local", "-path", filePath, "-service-names", ...instanceNames],
          undefined,
          undefined
        )
        .resolves(cliResult);
    ...
    it("ok:: serviceKeyNames are specified", async () => {
      ...
      cliMock
        ...
        .resolves(cliResult);
      await cfLocal.cfBindLocalServices(
        filePath,
        instanceNames,
        tags,
        keyNames
  ...
  describe("cfCreateService scope", () => {
    ...
    it("exception:: retryFunction returns incorrect result, verify the specified instance name is encoded", async () => {
      ...
      try {
        ...
        expect(e.message).to.be.equal(
          messages.service_not_found(requestedName)
        );
      }
    });
    ...
    it("exeption:: retryFunction returns incorrect result - instanceName not provided", async () => {
      ...
      cliMock
        ...
        .withExactArgs(
          [
            "curl",
            `/v3/service_instances?space_guids=${spaceGuid}&type=managed&per_page=297`,
          ],
          undefined,
          token
        )
    ...
    it("exception:: exceeded number of retry attempts", async () => {
      ...
      try {
        ...
        expect(e.message).to.be.equal(
          messages.exceed_number_of_attempts(instanceName)
        );
        expect(progressReported).to.be.true;
      }
    });
    ...
    it("exception:: operation state failed", async () => {
      ...
      try {
        ...
        expect(error.message).to.be.equal(
          messages.failed_creating_entity(description, instanceName)
        );
      }
    });
    ...
    it("exception:: creation service failed, have errors in output", async () => {
      ...
      try {
        ...
        expect(e.message).to.be.equal(
          messages.service_creation_failed(errorDetail)
        );
      }
    });
  });
  ...
  describe("cfGetServicePlansList scope", () => {
    ...
    it("ok:: cf space is not provided", async () => {
      ...
      cliMock
        .expects("execute")
        .withExactArgs(args, undefined, undefined)
        .resolves(cliResult);
    ...
    it("ok:: call where cf space is specified", async () => {
      ...
      cliMock
        ...
        .withExactArgs(
          [
            "curl",
            `/v3/service_plans?space_guids=${guid}&include=service_offering&per_page=${per_page}`,
          ],
          undefined,
          undefined
        )
    ...
    it("exception:: rejected error", async () => {
      ...
      cliMock
        ...
        .withExactArgs(
          [
            "curl",
            `/v3/service_credential_bindings?type=key&per_page=${CF_PAGE_SIZE}`,
          ],
          undefined,
          undefined
        )
