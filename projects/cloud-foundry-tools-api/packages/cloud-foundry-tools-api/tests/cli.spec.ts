...
describe("cli unit tests", () => {
  ...
  describe("execute", () => {
    ...
    it("stdout has no data, exit code 0", async () => {
      ...
      childProcessMock
        .expects("spawn")
        .withArgs("cf", undefined, undefined)
        .returns(execResult);
      const result = await Cli.execute(undefined, undefined, token);
    ...
    it("stderr has error data, exit code 2", async () => {
      ...
      execResult.on = (type: string, callback: any) =>
        type === "error"
          ? callback({ message: "test error message", code: 2 })
          : {};
      childProcessMock
        .expects("spawn")
        .withExactArgs("cf", undefined, undefined)
    ...
    it("stderr has error data, exit code ENOENT", async () => {
      execResult.on = (type: string, callback: any) =>
        type === "error"
          ? callback({ message: "test ENOENT message", code: "ENOENT" })
          : {};
      childProcessMock
        .expects("spawn")
        .withExactArgs("cf", undefined, undefined)
    ...
    it("stdout has error_code, code is 10002", async () => {
      ...
      execResult.on = (type: string, callback: any) =>
        type === "exit" ? callback(1) : {};
      execResult.stdout.on = (type: string, callback: any) =>
        callback(`{"type": "error_code", "code": 10002}`);
      childProcessMock
        .expects("spawn")
        .withExactArgs("cf", undefined, undefined)
        .returns(execResult);
    ...
    it("stdout has error_code, code is 404", async () => {
      ...
      execResult.stdout.on = (type: string, callback: any) =>
        callback(
          `{"type": "error_code", "code": 404, "description": "test error description"}`
        );
      childProcessMock
        .expects("spawn")
        .withExactArgs("cf", undefined, undefined)
    ...
    it("stdout starts with FAILED and has 'Error creating request'", async () => {
      execResult.on = (type: string, callback: any) =>
        type === "exit" ? callback(1) : {};
      execResult.stdout.on = (type: string, callback: any) =>
        callback("FAILED and has 'Error creating request'");
      childProcessMock
        .expects("spawn")
        .withExactArgs("cf", undefined, undefined)
        .returns(execResult);
      const result = await Cli.execute(undefined, undefined, token);
      expect(result.stdout).to.be.equal(
        "FAILED and has 'Error creating request'"
      );
      expect(result.stderr).to.be.empty;
      expect(result.exitCode).to.be.equal(-1);
      expect(result.error).to.be.equal(
    ...
    it("stdout command failed and has 'Error: some error occured'", async () => {
      ...
      const result = await Cli.execute(undefined, undefined, token);
      expect(result.stdout).to.be.equal(
        "Request is failed. Error: it is impossible to access db."
      );
      expect(result.stderr).to.be.empty;
      expect(result.exitCode).to.be.equal(-1);
      expect(result.error).to.be.equal(
