...
async function getTaskJsonContentAsJsonObject(
  taskJsonFilePath: string
): Promise<any> {
  try {
    const tasksJsonString = await fs.promises.readFile(taskJsonFilePath, {
      encoding: "utf8",
    });
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function saveTaskConfiguration(
  wsFolderPath: string,
  configuration: any
): Promise<void> {
  ...
  const existingTaskindex = tasksJson.tasks.findIndex(
    (task: any) => task.label === configuration.label
  );
