...
export class Contributors {
  ...
  public registerOnChangedCallback(
    thisArg: Object,
    callback: (collections: Array<IInternalCollection>) => void
  ): void {
      ...
      this.collectionsMap.get(extensionId).forEach((collection) => {
        ...
        collections.push({
          ...
          additionalInfo: collection.additionalInfo
            ? { ...collection.additionalInfo, ...iconInfo }
            : undefined,
        });
      });
    }
  ...
  public getCollectionsInfoOfTutorial(
    extensionId: string,
    tutorial: any
  ): Array<any> {
    const collections: Array<any> = [];
  ...
  public setData(
    ...
    if (this.onChangedCallback) {
      this.onChangedCallback.call(
        this.onChangedCallbackThis,
        this.getCollections()
      );
    }
  ...
  public init() {
    const allExtensions: readonly vscode.Extension<any>[] =
  ...
  private addCollections(
    extensionId: string,
    collections: Array<IInternalCollection>
  ) {
  ...
  private findCollction(collectionIdLower: string): IInternalCollection {
    ...
    for (const extensionId of this.collectionsMap.keys()) {
      if (collectionIdLower.startsWith(extensionId)) {
        result = this.collectionsMap
          .get(extensionId)
          .find(
            (collection) =>
              `${extensionId}.${collection.id.toLocaleLowerCase()}` ===
              collectionIdLower
          );
      }
    }
