...
export abstract class AbstractWebviewPanel {
  ...
  protected async initHtmlContent() {
    ...
    if (indexHtml) {
      ...
      indexHtml = indexHtml.replace(
        /<script defer="defer" src="/g,
        `<script defer="defer" src="${scriptUri.toString()}`
      );
      indexHtml = indexHtml.replace(
        /<script defer="defer" type="module" src="/g,
        `<script defer="defer" type="module" src="${scriptUri.toString()}`
      );
    }
