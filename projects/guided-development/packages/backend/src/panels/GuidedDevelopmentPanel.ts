...
export class GuidedDevelopmentPanel extends AbstractWebviewPanel {
  ...
  public loadWebviewPanel(uiOptions?: any) {
    if (
      this.webViewPanel &&
      JSON.stringify(uiOptions) === JSON.stringify(this.uiOptions)
    ) {
  ...
  public setWebviewPanel(webViewPanel: vscode.WebviewPanel, uiOptions?: any) {
    ...
    if (this.uiOptions && this.uiOptions.renderType) {
      ...
      this.collections.forEach((element) => {
        if (
          this.uiOptions.renderType === "collection" &&
          element.id === this.uiOptions.id
        ) {
          this.messages.collectionadditionalinfo = element.additionalInfo;
          let iconInfo =
            IconList[this.messages.collectionadditionalinfo?.iconCode];
          if (iconInfo && iconInfo.iconName) {
            this.messages.collectionadditionalinfo.iconName = iconInfo.iconName;
            this.messages.collectionadditionalinfo.iconLabel =
              iconInfo.iconLabel;
          }
    ...
    Contributors.getInstance().registerOnChangedCallback(
      this.guidedDevelopment,
      this.guidedDevelopment.setCollections
    );
  ...
  public static getOutputChannel(channelName: string): vscode.OutputChannel {
    if (!this.channel) {
      this.channel = vscode.window.createOutputChannel(
        `${GuidedDevelopmentPanel.GUIDED_DEVELOPMENT}.${channelName}`
      );
    }
