...
export async function activate(context: vscode.ExtensionContext) {
  ...
  registerAndSubscribeCommand(
    "loadGuidedDevelopment",
    guidedDevelopmentPanel.loadWebviewPanel.bind(guidedDevelopmentPanel)
  );
  registerAndSubscribeCommand(
    "guidedDevelopment.toggleOutput",
    guidedDevelopmentPanel.toggleOutput.bind(guidedDevelopmentPanel)
  );
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      GuideViewProvider.viewType,
      guideProvider,
      { webviewOptions: { retainContextWhenHidden: true } }
    )
  );
  registerAndSubscribeCommand(
    "guidedDevelopment.refreshCenter",
    guideProvider.refreshData.bind(guideProvider)
  );
