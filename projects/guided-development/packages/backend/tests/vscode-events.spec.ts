  ...
  if (action) {
    switch ((action as MockAction).actionType) {
      ...
      case "FILE":
        ...
        vscode.commands.executeCommand("vscode.open", uriAction.uri, {
...
describe("vscode-events unit test", () => {
  ...
  describe("performAction - on success", () => {
    it("Command as ActionType", async () => {
      commandsMock
        ...
        .withExactArgs("workbench.action.openGlobalSettings", [])
        .resolves();
  ...
  describe("setData", () => {
    it("contributor set data", () => {
      ...
      contributorsMock
        .expects("setData")
        .withExactArgs("extensionId", [collection1], [item1], undefined);
