...
const testVscode = {
  ...
  window: {
    ...
    registerWebviewViewProvider: (id: string, provider: any) => {
      _.set(oRegisteredWebviewViewProviders, id, provider);
      return Promise.resolve(oRegisteredWebviewViewProviders);
    },
  },
  extensions: {
...
describe("extension unit test", () => {
  ...
  describe("activate", () => {
    it("commands registration", async () => {
      ...
      expect(_.get(oRegisteredCommands, "loadGuidedDevelopment")).to.be.not
        .undefined;
      expect(_.get(oRegisteredCommands, "guidedDevelopment.toggleOutput")).to.be
        .not.undefined;
      expect(_.get(oRegisteredCommands, "guidedDevelopment.refreshCenter")).to
        .be.not.undefined;
    });
