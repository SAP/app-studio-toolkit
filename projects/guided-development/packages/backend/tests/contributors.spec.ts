...
describe("Contributors unit test", () => {
  ...
  describe("registerOnChangedCallback", () => {
    it("no guided development exists", () => {
      ...
      expect(Contributors.getInstance()["onChangedCallback"]).to.equal(
        undefined
      );
      expect(Contributors.getInstance()["onChangedCallbackThis"]).to.equal(
        undefined
      );
    });
  });
  ...
  describe("init", () => {
    ...
    it("Contributor exists", () => {
      ...
      expect(Contributors.getInstance()["collectionsMap"]).to.have.keys(
        "extid"
      );
    });
  ...
  describe("getItems", () => {
    it("collection has invalid itemId", () => {
      ...
      consoleMock
        .expects("error")
        .withExactArgs("Could not find item id itemId1");
  ...
  describe("initSubItems", () => {
    it("item has a subitem that does not exist", () => {
      ...
      expect(
        Contributors.getInstance()["itemsMap"].get("itemId1".toLowerCase())
      ).to.be.undefined;
    });
  });
