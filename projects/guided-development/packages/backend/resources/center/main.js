...
(function () {
  ...
  function updateGuideList(tutorials) {
    ...
    for (const tutorial of tutorials) {
      ...
      oTIcon.className =
        "tutorial-icon codicon codicon-view-pane-container-collapsed";
      ...
      for (const guide of tutorial.collections) {
        ...
        if (guide.additionalInfo?.iconName) {
          ...
          oSpan.className =
            "prefix-icon-span codicon codicon-" +
            guide.additionalInfo?.iconName;
      ...
      tli.addEventListener("click", (event) => {
        ...
        if (event.target === tli || event.target === oTLabel) {
          if (
            oTIcon.classList.contains("codicon-view-pane-container-collapsed")
          ) {
  ...
  function onGuideClicked(guide) {
    vscode.postMessage({
      type: "guideSelected",
      value: {
        renderType: guide.type ? guide.type : "collection",
        extensionId: guide.extensionId,
        id: guide.id,
        title: guide.title,
