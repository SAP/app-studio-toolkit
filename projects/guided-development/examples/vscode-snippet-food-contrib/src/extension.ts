...
let foodqCollectionTemplate: ICollection = {
  ...
  title: "Restaurants",
  description:
    "Find your desired restaurant from the options below. You can order take-away, delivery, or reserve a table. (The options rely on the 'foodq.json' file in your workspace.)",
  type: CollectionType.Scenario,
  itemIds: [`${EXT_ID}.foodq-restaurant`, `${EXT_ID}.michelin-restaurants`],
};
...
function getCollections(): ICollection[] {
  ...
  let collection: ICollection = {
    ...
    type: CollectionType.Scenario,
    itemIds: [`${EXT_ID}.create-grocery-list`],
  };
...
function getInitialItems(): Array<IItem> {
  ...
  item = {
    ...
    title: "Populate List",
    description:
      "Select the items you need to buy from the options provided.<br>\
    ...
    labels: [{ "Project Type": "create-grocery-list" }],
  };
  items.push(item);
  item = {
    id: "foodq-restaurant",
    title: "FoodQ",
    description:
      "Order your favorite dishes from the FoodQ restaurant.<br>\
    ...
    labels: [{ "Project Type": "foodq-restaurant" }],
  };
  items.push(item);
  item = {
    ...
    description:
      "Treat yourself to a first-class experience at our Michelin restaurants.<br>\
    ...
    labels: [{ "Project Type": "michelin-restaurants" }],
  };
  items.push(item);
  item = {
    ...
    description:
      'The Michelin guide defines 3-star restaurants as "exceptional cuisine that is worth a special journey". So embark in that journey and come enjoy the food.',
    image: {
      image: getImage(
        path.join(extensionPath, "resources", "Michelin-3-stars.png")
      ),
  ...
  item = {
    ...
    description:
      'The Michelin guide defines 2-star restaurants as "excellent cooking that is worth a detour". Select one of the restaurants in this list of a guaranteed delicious meal.',
    image: {
      image: getImage(
        path.join(extensionPath, "resources", "Michelin-2-stars.png")
      ),
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  basAPI
    .getExtensionAPI<ManagerAPI>("SAPOSS.guided-development")
    .then((managerAPI) => {
      ...
      let snippet: ISnippet = {
        getMessages() {
          return {
            title: "Create Grocery List",
            description:
              "Create an organized grocery list to avoid buying items you don't really need.",
            applyButton: "Create",
          };
        },
        async getQuestions() {
