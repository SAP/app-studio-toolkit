...
function getCollections(): ICollection[] {
  ...
  let collection: ICollection = {
    ...
    description:
      "This is a demo collection. It contains self-contributed items and and an item contributed by a different contributor.",
...
function getItems(): Array<IItem> {
  ...
  item = {
    ...
    itemIds: ["saposs.vscode-contrib1.open", "saposs.vscode-contrib2.cfLogin"],
    labels: [
      { "Project Name": "cap2" },
      { "Project Type": "CAP" },
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  showMessageAction.executeAction = () => {
    return vscode.window.showInformationMessage(
      "Hello from Open Global Settings item"
    );
  };
  ...
  cloneAction.executeAction = () => {
    return vscode.commands.executeCommand(
      "git.clone",
      "https://github.com/SAP/code-snippet.git"
    );
  };
  ...
  showInfoMessageAction.executeAction = () => {
    return vscode.window.showInformationMessage(
      "Hello from guided development item"
    );
  };
