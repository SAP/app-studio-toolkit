...
function initActions(basAPI: typeof bas) {
  ...
  eatAction.executeAction = () => {
    return vscode.window.showInformationMessage(
      "You are now ready to bake a cake."
    );
  };
  buyAction = new basAPI.actions.ExecuteAction();
  buyAction.executeAction = () => {
    return vscode.window.showInformationMessage(
      "The ingredients will be delivered to your house."
    );
  };
  ...
  bakeAction.executeAction = (params) => {
    if (params && params.length > 0) {
      return vscode.window.showInformationMessage(
        `Check if the cake is ready using a toothpick. (${params})`
      );
    } else {
      return vscode.window.showInformationMessage(
        `Check if the cake is ready using a toothpick.`
      );
    }
  };
  bakeItemAction = {
  ...
  pourAction.executeAction = () => {
    return vscode.window.showInformationMessage(
      "We recommend you use a round pan."
    );
  };
}
...
function getCollections(): ICollection[] {
  ...
  const bakeCollection: ICollection = {
    ...
    title: "Bake a Cake",
    description:
      "This is a repice for making baked cakes. You can bake a cake only if there is a bake.json file in your workspace",
    type: CollectionType.Scenario,
    itemIds: [
      `${EXT_ID}.buy-ingredients`,
      `${EXT_ID}.mix-ingredients`,
...
function getInitialItems(): Array<IItem> {
  ...
  let item: IItem = {
    ...
    title: "Eat Cake",
    description:
      "You're done!! The cake is ready to be enjoyed as the perfect dessert.",
    image: {
  ...
  item = {
    ...
    description:
      "Add the dry ingredients to a bowl, then add the liquid ingredients and mix them together.",
    action1: {
      name: "Mix",
      action: mixAction,
    },
  ...
  item = {
    ...
    description:
      "Pre-heat the oven. Leave the cake inside the oven for 45 minutes at a high temperature.",
    image: {
