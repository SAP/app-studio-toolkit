...
class InquirerUIPanel {
  ...
  private _getHtmlForWebview(webview: vscode.Webview) {
    // TODO: don't use sync
    let indexHtml: string = fs.readFileSync(
      path.join(this._extensionPath, "dist", "index.html"),
      "utf8"
    );
    if (indexHtml) {
      // Local path to main script run in the webview
      const scriptPathOnDisk = vscode.Uri.file(
        path.join(this._extensionPath, "dist", path.sep)
      );
      const scriptUri = this._panel.webview.asWebviewUri(scriptPathOnDisk);

      const $: cheerio.Root = cheerio.load(indexHtml);

      function replaceAttributePaths(
