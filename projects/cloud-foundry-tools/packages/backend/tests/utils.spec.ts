...
describe("utils unit tests", () => {
  ...
  describe("getEnvResources", () => {
    ...
    it("ok:: sanity with valid VCAP_SERVICES value surrounded with single quotes and spaces", async () => {
      ...
      const envFilePath: string = path.join(
        __dirname,
        "resources",
        ".testValidEnvVCAPWithQuotesAndSpaces"
      );
      const envResources = await utils.getEnvResources(envFilePath);
      const vcapServicesObj = envResources.vcapObject;
      expect(vcapServicesObj).to.not.be.empty;
  ...
  describe("Tests for removeResourceFromEnv when VCAP_SERVICES surrounded with quotes", () => {
    ...
    it("ok:: remove resource when VCAP is surrounded with single quotes and leave single quotes after removal", async () => {
      ...
      const vcapServicesObjAfterChange: string | null = PropertiesReader(
        envFilePath
      ).getRaw(utils.ENV_VCAP_RESOURCES);
      ...
      if (vcapServicesObjAfterChange) {
        ...
        expect(
          fail(
            "VCAP_SERVICES must exists and be empty after removing the last dependency"
          )
        );
      }
    });
  ...
  describe("Tests for removeResourceFromEnv", () => {
    ...
    it("ok:: remove resource when it is not tagged & verify it is left unquoted as it was.", async () => {
      ...
      const vcapProperty: string =
        envPropertiesAfterChange.getRaw(utils.ENV_VCAP_RESOURCES) ?? "";
    ...
    it("ok:: remove resource when it is not tagged by wrong type", async () => {
      ...
      await utils.removeResourceFromEnv(bindContext);
      assert.deepEqual(
        PropertiesReader(envFilePath).get(utils.ENV_VCAP_RESOURCES),
        vcapServicesObjBeforeChange
      );
    });
    ...
    it("ok:: remove resource when it has a tag", async () => {
      ...
      const envPropertiesAfterChange = PropertiesReader(envFilePath);
      expect(
        envPropertiesAfterChange.get(utils.ENV_VCAP_RESOURCES)
      ).to.not.include('"instance_name":"hdi-test-instance"');
      expect(envPropertiesAfterChange.get(utils.ENV_VCAP_RESOURCES)).to.include(
        "xsuaa"
      );
    });
    ...
    it("ok:: remove resource when it has a tag, resource type has more then 1 instance (e.g. two dependencies bound to different instances)", async () => {
      ...
      expect(envPropertiesAfterChange.get(utils.ENV_VCAP_RESOURCES)).to.include(
        '"instance_name":"hana-2"'
      );
      expect(
        envPropertiesAfterChange.get(utils.ENV_VCAP_RESOURCES)
      ).to.not.include('"instance_name":"hana-1"');
      ...
      expect(
        envPropertiesAfterChange.get(utils.ENV_VCAP_RESOURCES)
      ).to.not.include("hanatest");
      expect(
        envPropertiesAfterChange.get(utils.ENV_VCAP_RESOURCES)
      ).to.not.include('"instance_name":"hana-2"');
      expect(
        envPropertiesAfterChange.get(utils.ENV_VCAP_RESOURCES)
      ).to.not.include('"instance_name":"hana-1"');
    });
  ...
  describe("API for creation service", () => {
    ...
    it("generateParams4Service - xsuaa, plan - application", () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
      expect(_.size(data["oauth2-configuration"]["redirect-uris"])).to.be.equal(
        1
      );
    });
    ...
    it("validateParams - other : valid", () => {
      expect(utils.validateParams("other")('{ "key" : "value" }')).to.be
        .undefined;
    });

    it("validateParams - other : incorrect", () => {
      expect(
        _.size(utils.validateParams("other")('{ "key" :: "value" }'))
      ).to.be.gt(1);
    });

    it("validateParams - xsuaa : valid", () => {
      const xsuaaData = utils.generateParams4Service("xsuaa", plan);
      expect(utils.validateParams("xsuaa", plan)(stringify(xsuaaData))).to.be
        .undefined;
    });
    ...
    it("validateParams - xsuaa : invalid xsappname", () => {
      ...
      expect(
        _.size(utils.validateParams("xsuaa", plan)(stringify(xsuaaData)))
      ).to.be.gt(1);
    });
    ...
    it("validateParams - xsuaa : invalid redirect-uris", () => {
      const xsuaaData = utils.generateParams4Service("xsuaa", plan);
      xsuaaData["oauth2-configuration"]["redirect-uris"] = [
        "localhost://*",
        "localhos://*",
      ];
      expect(
  ...
  describe("suite for getAllServiceInstances", () => {
    const services: ServiceInstanceInfo[] = [
      {
        ...
        serviceName: "hana",
        plan: "s-plan-1",
        plan_guid: "s-plan-guid-1",
      },
      {
        label: "s1",
        serviceName: "hana",
        plan: "s-plan-2",
        plan_guid: "s-plan-guid-2",
      },
    ];
    const ups = [
      {
        ...
        credentials: { tags: ["monodb", "hana"] },
      },
      {
        label: "ups2",
        serviceName: "user-provided-service",
        credentials: { tags: ["david"] },
      },
    ];
    ...
    it("ok:: there are both kind of services", async () => {
      ...
      assert.deepEqual(
        await utils.getAllServiceInstances({ ups: { isShow: true } }),
        _.concat(services, ups)
      );
    });
    ...
    it("ok:: contain 'managed' and 'ups' filtered services", async () => {
      ...
      sandbox.stub(cfLocal, "cfGetUpsInstances").resolves(ups);
      assert.deepEqual(
        await utils.getAllServiceInstances({ ups: { tag: "/.*dav.*/" } }),
        _.concat(services, ups[1])
      );
    });
    ...
    it("ok:: there are ups filtered services", async () => {
      ...
      expectedQuery.filters = [
        { key: eFilters.space_guids, value: "testSpace" },
      ];
      sandbox.stub(cfLocal, "cfGetManagedServiceInstances").resolves(services);
      sandbox
        .stub(cfLocal, "cfGetUpsInstances")
        .withArgs(expectedQuery)
        .resolves(ups);
      assert.deepEqual(
        await utils.getAllServiceInstances(opts),
        _.concat(services, ups[0])
      );
    });
  });
  ...
  describe("getUpsServiceInstances", () => {
    const ups = [
      ...
      },
      {
        label: "ups3",
        serviceName: "user-provided-service",
        credentials: { tags: "hana" },
      },
      {
        label: "wrong-credential-tags",
        serviceName: "user-provided-service",
        credentials: { tags: 2 },
      },
      {
    ...
    it("ok:: get all user provides services filtered by credential tag", async () => {
      ...
      sandbox.stub(cfLocal, "cfGetUpsInstances").resolves(ups);
      assert.deepEqual(await utils.getUpsServiceInstances(options), [
        ups[0],
        ups[2],
      ]);
    });
  ...
  describe("updateGitIgnoreList suite", () => {
    const project = {
      uri: nsVsMock.testVscode.Uri.file(path.resolve("myProject")),
    };
    const ignorePattern = new nsVsMock.testVscode.RelativePattern(
      project,
      GITIGNORE
    );
    const gitIgnoreFile = nsVsMock.testVscode.Uri.file(
      path.resolve(`${project.uri.fsPath}/${GITIGNORE}`)
    );

    it("ok:: '.env' pattern added", async () => {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
      const gitIgnoreContent = _.join(["     # empty line"], EOL);
      const testEnv = nsVsMock.testVscode.Uri.file(
        path.resolve(`${project.uri.fsPath}/subFolder/.env`)
      );
      vscodeWorkspaceMock.expects("getWorkspaceFolder").returns(project);
      vscodeWorkspaceMock
        .expects("findFiles")
        .withExactArgs(ignorePattern)
        .returns([gitIgnoreFile]);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
      fsMock
        .expects("readFile")
        .withExactArgs(gitIgnoreFile.fsPath, { encoding: UTF8 })
        .resolves(gitIgnoreContent);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
      const patterns = _.split(gitIgnoreContent, EOL);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
      const expectedPatterns = _.join(
        _.concat(patterns, [`# auto generated wildcard`, "subFolder/.env"]),
        EOL
      );
      fsMock
        .expects("writeFile")
        .withExactArgs(gitIgnoreFile.fsPath, expectedPatterns, {
          encoding: UTF8,
        })
        .resolves(undefined);
    ...
    it("ok:: '.env' pattern not added - already exists", async () => {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
      const gitIgnoreContent = _.join(
        ["     # empty line", "subFolder/.env"],
        EOL
      );
      const testEnv = nsVsMock.testVscode.Uri.file(
        path.resolve(`${project.uri.fsPath}/subFolder/.env`)
    ...
    it("ok:: '.env' pattern added, .gitIgnore file not exist", async () => {
      ...
      fsMock
        .expects("open")
        .withExactArgs(path.resolve(gitIgnoreFile.fsPath), "w")
        .resolves(undefined);
      fsMock
        .expects("readFile")
        .withExactArgs(gitIgnoreFile.fsPath, { encoding: UTF8 })
    ...
    describe("notifyWhenServicesInfoResultIncomplete method", () => {
      ...
      it("ok:: serviceInstanceInfo list is fullfilled", async () => {
        ...
        expect(
          await utils.notifyWhenServicesInfoResultIncomplete(
            Promise.resolve(infos)
          )
        ).to.be.deep.equal(infos);
      });
      ...
      it("ok:: serviceInstanceInfo list is empty", async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        expect(
          await utils.notifyWhenServicesInfoResultIncomplete(list)
        ).to.be.deep.equal([]);
      });
      ...
      it("ok:: serviceInstanceInfo list is incomplete", async () => {
        ...
        vscodeWindowMock
          ...
          .withExactArgs(
            messages.messages.service_instances_list_incomplete(
              _.map(infos, "label")
            )
          )
          .resolves();
    ...
    describe("verifyCFTarget", () => {
      ...
      it("ok:: validate", async () => {
        ...
        expect(await utils.examCFTarget("", ["user"], false)).to.be.deep.equal(
          target
        );
      });
      ...
      it("ok:: weak mode", async () => {
        ...
        expect(
          await utils.examCFTarget("", ["api endpoint"], true)
        ).to.be.deep.equal(t);
      });
