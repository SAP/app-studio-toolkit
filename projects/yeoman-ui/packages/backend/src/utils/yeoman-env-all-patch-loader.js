...
module.exports = function yeomanEnvAllPatchLoader(source) {
  ...
  result = result.replace(
    ...
    `const ENVIRONMENT_VERSION = ${JSON.stringify(
      pkg.version
    )}; // inlined by yeoman-env-all-patch-loader`
  );
  ...
  result = result.replace(
    /\brequire\.resolve\(meta\.resolved\)/g,
    "__non_webpack_require__.resolve(meta.resolved)"
  );
  result = result.replace(
    /\brequire\(meta\.resolved\)/g,
    "__non_webpack_require__(meta.resolved)"
  );
  ...
  result = result.replace(
    ...
    `const PACKAGE_NAME_PATTERN = [${JSON.stringify(
      pkg.name
    )}]; // inlined by yeoman-env-all-patch-loader`
  );
