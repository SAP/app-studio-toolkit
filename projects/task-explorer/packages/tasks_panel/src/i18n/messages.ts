export const messages = {
  ...
  GET_TREE_BRANCHES: (name: string, intents: number): string =>
    `Retrieved ${intents} ${name}.`,
  ...
  EDIT_TASK: (task: string): string =>
    `The changes to the ${task} task have been saved.`,
  ...
  DUPLICATED_TYPE: (type: string): string =>
    `The "${type}" type has already been contributed.`,
  EXECUTE_FAILURE: (task: string): string => `Could not execute the "${task}"`,
  TERMINATE_FAILURE: (task: string, reason: string): string =>
    `Could not terminate the ${task} task. ${reason}`,
  ...
  delete_task_confirmation: (name: string) =>
    `Are you sure you want to delete the '${name}' configuration?`,
  resource_open_could_not_open_editor:
    "Could not open the editor for the required resource",
  configuration_task_not_found: (label: string) =>
    `Could not find the "${label}" configuration.`,
  create_task_pick_project_placeholder:
    "Select the project for which you want to create the task:",
  create_task_pick_task_placeholder: "Select the task you want to perform:",
  err_task_definition_unsupported_target:
    "Unable to complete the tasks definition - unsupported deployment target",
};
