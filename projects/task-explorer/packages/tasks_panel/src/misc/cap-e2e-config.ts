...
export async function getCapE2ePickItems(
  info: ProjectInfo
): Promise<CapProjectConfigInfo | undefined> {
  ...
  async function isConfigured(): Promise<boolean> {
    if (
      await doesFileExist(
        Uri.joinPath(Uri.file(info.wsFolder), info.project, "mta.yaml")
      )
    ) {
      const tasksPattern = map(
        await generateMtaDeployTasks(info.wsFolder, info.project, "sequence"),
        (task) => {
...
export async function capE2eConfig(data: {
  wsFolder: string;
  project: string;
}): Promise<void> {
  ...
  if (
    !(await doesFileExist(
      Uri.joinPath(Uri.file(data.wsFolder), data.project, "mta.yaml")
    ))
  ) {
  ...
  if (await areResourcesReady(resourcesPromises)) {
    const mtaDeployTasks = await generateMtaDeployTasks(
      data.wsFolder,
      data.project
    );
