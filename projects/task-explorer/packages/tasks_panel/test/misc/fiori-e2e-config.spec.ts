...
} from "../../src/misc/fiori-e2e-config";
...
`;
  ...
  describe("getFioriE2ePickItems scope", () => {
    ...
    it("getFioriE2ePickItems - 'ui5-deploy.yaml' does not exist, config required", async () => {
      mockCommands
        .expects("getCommands")
        .resolves(["sap.ux.appGenerator.launchDeployConfig"]);
      mockWorkspaceFs
        .expects("stat")
  ...
  describe("fioriE2eConfig scope", () => {
    ...
    beforeEach(() => {
      mockCommands
        ...
        .withExactArgs("sap.ux.appGenerator.launchDeployConfig", {
          fsPath: testVscode.Uri.joinPath(
            testVscode.Uri.file(wsFolder),
            project
          ).fsPath,
        })
      ...
      mockWorkspace
        ...
        .withExactArgs(
          new testVscode.RelativePattern(
            wsFolder,
            path.join(path.sep, "project", "ui5-deploy.yaml")
          ),
          false,
          false,
          true
    ...
    it("fioriE2eConfig, deploy.yaml is updated, target ABAP, task defined", async () => {
      ...
      const tasks = (mockE2eConfig as any).expectations["addTaskDefinition"][0]
        .args[0][1];
      expect(tasks.length).be.eq(1);
      // verify arg of `tasks-explorer.editTask` command
      expect(
        (mockCommands as any).expectations["executeCommand"][1].args[0][1]
      ).be.deep.equal(tasks[0]);
      // verify arg of `tasks-explorer.tree.select` command
      expect(
        (mockCommands as any).expectations["executeCommand"][2].args[0][1]
      ).be.deep.equal(tasks[0]);
    ...
    it("fioriE2eConfig, deploy.yaml is updated, target CF, tasks defined", async () => {
      ...
      expect(/^Deploy to Cloud Foundry/.test((tasks[1] as any).label)).to.be
        .true;
      ...
      expect((tasks[1] as any).dependsOn).to.be.deep.equal([
        `${tasks[0].label}`,
      ]);
    });
    ...
    it("fioriE2eConfig, deploy.yaml is updated, target unexpected", async () => {
      ...
      mockWorkspaceFs
        .expects("readFile")
        .withExactArgs(uriUi5DeployYaml)
        .resolves(Buffer.from("", `utf8`));
      try {
        await fioriE2eConfig(data);
        fail("should fail");
      } catch (e: any) {
        expect(e.message).to.be.equal(
          messages.err_task_definition_unsupported_target
        );
      }
    });
  });
  ...
  describe("fioriE2eConfig scope - single root", () => {
    ...
    beforeEach(() => {
      ...
      mockWorkspace
        .expects("createFileSystemWatcher")
        .withExactArgs(
          new testVscode.RelativePattern(wsFolder, "ui5-deploy.yaml"),
          false,
          false,
          true
        )
