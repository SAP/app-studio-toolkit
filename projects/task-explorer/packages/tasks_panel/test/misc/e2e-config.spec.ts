...
describe("e2e-config scope", () => {
  ...
  describe("waitForResource scope", () => {
    ...
    it("waitForResource - onDidChange callback triggered", async () => {
      mockWorkspace
        ...
        .withExactArgs(pattern, false, false, true)
        .returns(watcher);
    ...
    it("waitForResource - onDidCreate callback triggered", async () => {
      mockWorkspace
        .expects("createFileSystemWatcher")
        .withExactArgs(pattern, false, false, false)
        .returns(watcher);
      setTimeout(() => callbacks["create"]());
      expect(await waitForFileResource(pattern, false, false, false)).to.be
        .true;
    });
  ...
  describe("collectProjects", () => {
    ...
    const projectInfo = {
      path: testVscode.Uri.joinPath(testVscode.Uri.file(wsFolder), "project")
        .path,
      type: "com.sap.fe",
    };
    const infoProject = {
    ...
    it("collectProjects - toolkit extension is not configured well", async () => {
      mockExtensions
        ...
        .withExactArgs("SAPOSS.app-studio-toolkit")
        .returns({
          exports: null,
        });
    ...
    it("collectProjects - project found, projectInfo undefined", async () => {
      ...
      sandbox
        .stub(btaExtension.exports.workspaceAPI, "getProjects")
        .resolves([infoProject]);
      mockExtensions
        .expects("getExtension")
        .withExactArgs("SAPOSS.app-studio-toolkit")
        .returns(btaExtension);
      ...
      mockWorkspace
        ...
        .returns({
          uri: testVscode.Uri.file(
            path.join(path.sep, "home", "user", "test1")
          ),
        });
      expect(await collectProjects(wsFolder, true)).to.be.deep.equal([]);
    });
    ...
    it("collectProjects - project found, but the project type does not match the 'com.sap.fe' type", async () => {
      ...
      mockWorkspace
        ...
        .withExactArgs(testVscode.Uri.file(projectInfo.path))
        .returns({
          uri: testVscode.Uri.file(path.join(path.sep, "home", "user", "test")),
        });
      expect(await collectProjects(wsFolder, true)).to.be.deep.equal([]);
    });
    ...
    it("collectProjects - project found, project type is 'com.sap.fe' type", async () => {
      ...
      mockWorkspace
        ...
        .returns({
          uri: testVscode.Uri.file(
            path.join(path.sep, "home", "user", "test", path.sep)
          ),
        });
      expect(await collectProjects(wsFolder, true)).to.be.deep.equal([
        {
    ...
    it("collectProjects - project opened as a root, project type is 'com.sap.fe' type", async () => {
      ...
      mockExtensions
        ...
        .returns(btaExtension);
      mockWorkspace
        .expects("asRelativePath")
        .withExactArgs(folder, false)
        .returns(folder);
      mockWorkspace
        .expects("getWorkspaceFolder")
    ...
    it("collectProjects - project found, project type is 'com.sap.hana' type", async () => {
      ...
      const projectInfo = {
        path: testVscode.Uri.joinPath(testVscode.Uri.file(wsFolder), "proj")
          .path,
        type: "com.sap.hana",
      };
      sandbox.stub(infoProject, "getProjectInfo").resolves(projectInfo);
      ...
      mockWorkspace
        .expects("asRelativePath")
        .withExactArgs(projectInfo.path, false)
        .returns("proj");
      expect(await collectProjects(wsFolder, true)).to.be.deep.equal([
        {
          wsFolder,
  ...
  describe("addTaskDefinition scope", () => {
    ...
    beforeEach(() => {
      ...
      mockUpdateConfiguration
        .expects("updateTasksConfiguration")
        .withArgs(wsFolder)
        .resolves();
    });
    ...
    it("addTaskDefinition - get `tasks` returns `undefined`", async () => {
      ...
      const _tasks = (<any>mockUpdateConfiguration).expectations[
  ...
  describe("isFileExist scope", () => {
    const url = testVscode.Uri.file(
      path.join(path.sep, "home", "user", "test", "project", "my", "file.txt")
    );
    ...
    it("isFileExist - does exist", async () => {
      mockWorkspaceFs
        .expects("stat")
        .withExactArgs(url)
        .resolves({ type: testVscode.FileType.File });
  ...
  describe("generateMtaDeployTasks scope", () => {
    ...
    const projectUri = testVscode.Uri.joinPath(
      testVscode.Uri.file(wsFolder),
      project
    );
    const taskBuild = {
    ...
    it("generateMtaDeployTasks - no cf target defined", async () => {
      ...
      expect(await generateMtaDeployTasks(wsFolder, project)).be.deep.equal([
        ...
        {
          ...taskDeploy,
          ...{ cfTarget: "", cfEndpoint: "", cfOrg: "", cfSpace: "" },
        },
      ]);
    });
    ...
    it("generateMtaDeployTasks - cf target found", async () => {
      ...
      mockCfTools
        .expects("cfGetTargets")
        .resolves([{ label: targetName, isCurrent: true }]);
      expect(await generateMtaDeployTasks(wsFolder, project)).be.deep.equal([
        taskBuild,
        {
          ...taskDeploy,
    ...
    it("generateMtaDeployTasks - sequence label type, cf target found", async () => {
      ...
      expect(
        await generateMtaDeployTasks(wsFolder, "", "sequence")
      ).be.deep.equal([
        { ...copyTaskBuild, ...{ projectPath: `${wsFolder}` } },
        {
          ...
          ...{ cfTarget: targetName, cfEndpoint: "", cfOrg: "", cfSpace: "" },
          ...{
            mtarPath: path.normalize(
              path.join(wsFolder, "mta_archives", "project_0.0.1.mtar")
            ),
          },
        },
      ]);
  ...
  describe("calculateTaskWsFolder scope", () => {
    ...
    it("task type - 'npm', 'path' property found", async () => {
      ...
      expect(calculateTaskWsFolder(task)).be.equal(
        path.join(task.__wsFolder, "test-path")
      );
    });
