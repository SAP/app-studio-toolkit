...
describe("extension", () => {
  describe("activate method", () => {
    ...
    it("verify 'refresh' callback configured", async () => {
      await activate(testVscode.ExtensionContext);
      const mockTreeProvider = mock(
        MockVSCodeInfo.treeDataProvider.get("tasksPanel")
      );
  ...
  describe("package definitions", () => {
    ...
    it("extension pack contributes viewsWelcome verifing", () => {
      ...
      expect(
        find(contents, (line) =>
          /\[.*\]\(command:tasks-explorer.createTask\)/.test(line)
        )
      ).to.be.ok;
    });
    ...
    it("extension pack contributes menus->view/item/context", () => {
      const item = find(packageJson.contributes.menus["view/item/context"], [
        "command",
      ...
      expect(map(split(statements[0], "=="), trim).includes("tasksPanel")).to.be
        .true;
      expect(map(split(statements[1], "=~"), trim).includes("viewItem")).to.be
        .true;
      expect(
        map(split(statements[1], "=~"), trim).includes(
          "/^(root|project|intent)$/"
        )
      ).to.be.true;
    });
    ...
    it("extension pack contributes menus->view/title createTask command", () => {
      const item = find(packageJson.contributes.menus["view/title"], [
        "command",
        "tasks-explorer.createTask",
      ]);
      expect(item?.group).to.be.equal("navigation");
      expect(map(split(item?.when, "=="), trim).includes("view")).to.be.true;
      expect(map(split(item?.when, "=="), trim).includes("tasksPanel")).to.be
        .true;
    });
